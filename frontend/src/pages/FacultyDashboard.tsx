...
import { Award, Sparkles, Plus, BookOpen, Users, Building2, Calendar } from 'lucide-react';
...
export const FacultyDashboard: React.FC = () => {
  ...
  if (loading || !profile) {
    return <div className="p-8 text-center py-20 text-slate-500 font-medium animate-pulse">Loading Faculty Collaboration Hub...</div>;
  }
      ...
      <div className="bg-gradient-to-r from-purple-900 to-slate-900 text-white rounded-3xl p-6 shadow-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-1">
          <div className="inline-flex items-center space-x-2 bg-purple-800/60 text-purple-200 text-xs font-semibold px-3 py-1 rounded-full border border-purple-600/40">
            ...
            <span>Faculty & Academician Hub</span>
          </div>
          <h1 className="text-2xl font-extrabold">{profile.full_name}</h1>
          <p className="text-xs text-slate-300">{profile.designation} • {profile.department} ({profile.institution_name})</p>
        </div>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 flex items-center space-x-2">
            ...
            <span>Academia–Industry Collaboration Opportunities</span>
          </h2>
          <p className="text-xs text-slate-500">FDPs, Consultancy, Research Projects, Workshops, and Guest Lectures</p>
        </div>
